...
#define IRECOVERY_SEND_CHUNK_SIZE 512
...
struct irecovery_client {
	...
	irecovery_event_cb_t appvar_progress_callback;   // Appvar progress callback
};
...
static irecovery_error_t irecovery_finalize_client(irecovery_client_t client) {
    ...
    if (ret < 0) {
        ...
        if (client->mode == IRECOVERY_K_WTF_MODE) {
			// WTF mode serial is just the raw CPID as a decimal string
			char cpid_str[5] = {0};
			strncpy(cpid_str, serial_str, sizeof(cpid_str)-1);
			unsigned int cpid = 0;
			nsscanf(cpid_str, "%x", &cpid);
			client->device_info.cpid = cpid;
			client->device_info.pid  = client->device_descriptor.idProduct;
			client->finalized        = 1;
			return IRECOVERY_E_SUCCESS;
		} else {
...
const char* irecovery_strerror(irecovery_error_t error) {
    switch (error) {
		...
		case IRECOVERY_E_NO_VARIABLE:
			return "There is no variable to set.";
		case IRECOVERY_E_NO_VALUE:
			return "There is no value to set.";
		case IRECOVERY_E_FILE_NOT_FOUND:
			return "The provided filename was not found.";
		case IRECOVERY_E_FILE_SIZE_ZERO:
			return "The provided file's size is zero.";
		case IRECOVERY_E_FILE_READ_ERROR:
			return "Failed to read from the provided file.";
		case IRECOVERY_E_TRANSFER_STOPPED_BY_CALLBACK:
			return "The transfer was manually stopped.";
        default:
...
int irecovery_usb_bulk_transfer(irecovery_client_t client, unsigned char endpoint, unsigned char* data, size_t length, size_t* transferred) {	
    ...
    if (error != USB_SUCCESS) {
		usb_ClearEndpointHalt(usb_GetDeviceEndpoint(client->handle, endpoint));
...
irecovery_error_t irecovery_event_subscribe(irecovery_client_t client, irecovery_event_type type, irecovery_event_cb_t callback) {
	...
	switch (type) {
		...
		case IRECOVERY_APPVAR_PROGRESS:
			client->appvar_progress_callback = callback;
			break;
...
irecovery_error_t irecovery_event_unsubscribe(irecovery_client_t client, irecovery_event_type type) {
	...
	switch (type) {
		...
		case IRECOVERY_APPVAR_PROGRESS:
			client->appvar_progress_callback = NULL;
			break;
...
irecovery_error_t irecovery_send_buffer(irecovery_client_t client, unsigned char* buffer, size_t length, unsigned int options) {
    ...
    static uint32_t h1 = 0xFFFFFFFF;
    static unsigned char dfu_xbuf[12] = {0xff, 0xff, 0xff, 0xff, 0xac, 0x05, 0x00, 0x01, 0x55, 0x46, 0x44, 0x10};
    static bool dfu_crc = true;
    static bool transfer_initiated = false;
    static size_t global_block_offset = 0;

    irecovery_error_t error = IRECOVERY_E_SUCCESS;
    bool recovery_mode = ((client->mode == IRECOVERY_K_RECOVERY_MODE_1) ||
                          (client->mode == IRECOVERY_K_RECOVERY_MODE_2) ||
                          (client->mode == IRECOVERY_K_RECOVERY_MODE_3) ||
                          (client->mode == IRECOVERY_K_RECOVERY_MODE_4));

    bool chunked = (options & IRECOVERY_SEND_OPT_DFU_CRC_RETAIN) || (options & IRECOVERY_SEND_OPT_DFU_CRC_FINALIZE);
    if (!chunked) {
        h1 = 0xFFFFFFFF;
        memcpy(dfu_xbuf, (unsigned char[]){0xff, 0xff, 0xff, 0xff, 0xac, 0x05, 0x00, 0x01, 0x55, 0x46, 0x44, 0x10}, 12);
        dfu_crc = true;
    ...
    if (!recovery_mode && (options & IRECOVERY_SEND_OPT_DFU_SMALL_PKT)) {
        packet_size = 0x40;
        dfu_crc = false;
    }
    size_t last = length % packet_size;
    size_t packets = length / packet_size;
    ...
    bool is_first_chunk = !chunked || (!transfer_initiated && (options & IRECOVERY_SEND_OPT_DFU_CRC_RETAIN));
    if (is_first_chunk) {
        if (recovery_mode) {
            ...
            switch (state) {
                ...
                case 10:
                    ...
                    break;
                default:
                    irecovery_log(client, "Unexpected state %" PRIu8 ", issuing ABORT\n", state);
    ...
    for (size_t i = 0; i < packets; i++) {
        ...
        size_t block_num = global_block_offset + i;
        ...
        if (recovery_mode) {
            ...
            bool is_last_packet = (i + 1 == packets) && (!chunked || (options & IRECOVERY_SEND_OPT_DFU_CRC_FINALIZE));

            if (dfu_crc && is_last_packet) {
                if (size + 16 > packet_size) {
                    bytes = irecovery_usb_control_transfer(client, 0x21, 1, block_num, 0, &buffer[i * packet_size], size);
                    if (bytes != size) return IRECOVERY_E_USB_UPLOAD_FAILED;
                    count += size;
                    size = 0;
                    global_block_offset++;
                    block_num = global_block_offset + i;
                }
                for (int j = 0; j < 2; j++) {
                ...
                size += 16;
                bytes = irecovery_usb_control_transfer(client, 0x21, 1, block_num, 0, (unsigned char*)newbuf, size);
                free(newbuf);
            } else {
                bytes = irecovery_usb_control_transfer(client, 0x21, 1, block_num, 0, &buffer[i * packet_size], size);
            }
        ...
        if (client->progress_callback != NULL) {
            ...
            event.type = IRECOVERY_PROGRESS;
            event.data = (char*)"Uploading";
            event.size = count;
            if (client->progress_callback(client, &event) != 0) return IRECOVERY_E_TRANSFER_STOPPED_BY_CALLBACK;
        } else {
            irecovery_log(client, "Sent %zu bytes - %zu of %zu\n", bytes, count, length);
        }
    }

    global_block_offset += packets;

    if (recovery_mode && length % 512 == 0 && (options & IRECOVERY_SEND_OPT_DFU_CRC_FINALIZE)) {
		bytes = 0;
		irecovery_usb_bulk_transfer(client, 0x04, buffer, 0, &bytes);
	}

    if ((options & IRECOVERY_SEND_OPT_DFU_NOTIFY_FINISH) && !recovery_mode) {
        irecovery_usb_control_transfer(client, 0x21, 1, global_block_offset, 0, (unsigned char*) buffer, 0);
        ...
        if ((options & IRECOVERY_SEND_OPT_DFU_FORCE_ZLP)) {
            irecovery_usb_control_transfer(client, 0x21, 1, 0, 0, 0, 0);
        }

        irecovery_reset(client);
    }

    if (options & IRECOVERY_SEND_OPT_DFU_CRC_FINALIZE) {
        h1 = 0xFFFFFFFF;
        memcpy(dfu_xbuf, (unsigned char[]){0xff, 0xff, 0xff, 0xff, 0xac, 0x05, 0x00, 0x01, 0x55, 0x46, 0x44, 0x10}, 12);
        dfu_crc = true;
        global_block_offset = 0;
...
irecovery_error_t irecovery_send_appvar(const char* filename, irecovery_client_t client, unsigned int options, bool more_after_this) {
	if (!irecovery_client_is_usable(client, true)) {
        return IRECOVERY_E_NO_DEVICE;
    } else if (!filename) {
        return IRECOVERY_E_BAD_PTR;
	...
	static uint8_t leftover[IRECOVERY_SEND_CHUNK_SIZE];
	static size_t leftover_size = 0;

	uint8_t fp = ti_Open(filename, "r");
	if (!fp) return IRECOVERY_E_FILE_NOT_FOUND;

	size_t total_size = ti_GetSize(fp);
	if (total_size == 0) {
		ti_Close(fp);
		return IRECOVERY_E_FILE_SIZE_ZERO;
	}

	size_t bytes_remaining = total_size;
	size_t total_sent = 0;
	uint8_t chunk[IRECOVERY_SEND_CHUNK_SIZE];
	size_t chunk_offset = 0;
	irecovery_error_t err = IRECOVERY_E_SUCCESS;

	memset(chunk, 0, sizeof(chunk));

	if (leftover_size > 0) {
		memcpy(chunk, leftover, leftover_size);
		chunk_offset = leftover_size;
		leftover_size = 0;
	}

	while (bytes_remaining > 0) {
		size_t space = IRECOVERY_SEND_CHUNK_SIZE - chunk_offset;
		size_t read_size = bytes_remaining > space ? space : bytes_remaining;

		if (ti_Read(chunk + chunk_offset, read_size, 1, fp) != 1) {
			ti_Close(fp);
			memset(leftover, 0, sizeof(leftover));
			memset(chunk, 0, sizeof(chunk));
			leftover_size = 0;
			return IRECOVERY_E_FILE_READ_ERROR;
		}

		chunk_offset += read_size;
		bytes_remaining -= read_size;

		bool is_last = (bytes_remaining == 0);

		if (chunk_offset < IRECOVERY_SEND_CHUNK_SIZE && is_last && more_after_this) {
			memcpy(leftover, chunk, chunk_offset);
			leftover_size = chunk_offset;
			memset(chunk, 0, sizeof(chunk));
			ti_Close(fp);
			return IRECOVERY_E_SUCCESS;
		}

		if (chunk_offset == IRECOVERY_SEND_CHUNK_SIZE || is_last) {
			unsigned int chunk_options = options;

			if (!is_last || more_after_this) {
				chunk_options |= IRECOVERY_SEND_OPT_DFU_CRC_RETAIN;
			} else {
				chunk_options |= IRECOVERY_SEND_OPT_DFU_NOTIFY_FINISH;
				chunk_options |= IRECOVERY_SEND_OPT_DFU_CRC_FINALIZE;
			}

			err = irecovery_send_buffer(client, chunk, chunk_offset, chunk_options);
			...
			total_sent += chunk_offset;
			chunk_offset = 0;
			memset(chunk, 0, sizeof(chunk));

			if (client->appvar_progress_callback != NULL) {
				irecovery_event_t event;
				event.progress = ((double) total_sent / (double) total_size) * 100.0;
				event.type = IRECOVERY_PROGRESS;
				event.data = (char*)"Uploading";
				event.size = total_size;
				if (client->appvar_progress_callback(client, &event) != 0) {
					ti_Close(fp);
					memset(leftover, 0, sizeof(leftover));
					memset(chunk, 0, sizeof(chunk));
					leftover_size = 0;
					return IRECOVERY_E_TRANSFER_STOPPED_BY_CALLBACK;
				}
			} else {
				irecovery_log(client, "Sent %zu bytes - %zu of %zu\n", chunk_offset, total_sent, total_size);
			}
		}
	}
...
irecovery_error_t irecovery_setenv(irecovery_client_t client, const char* variable, const char* value) {
	...
	if (strlen(variable) == 0) {
		return IRECOVERY_E_NO_VARIABLE;
	} else if (strlen(value) == 0) {
		return IRECOVERY_E_NO_VALUE;
	}
