...
typedef enum {
    ...
    IRECOVERY_E_NO_VARIABLE                  = -19,
    IRECOVERY_E_NO_VALUE                     = -20,
    IRECOVERY_E_FILE_NOT_FOUND               = -21,
    IRECOVERY_E_FILE_SIZE_ZERO               = -22,
    IRECOVERY_E_FILE_READ_ERROR              = -23,
    IRECOVERY_E_TRANSFER_STOPPED_BY_CALLBACK = -24
} irecovery_error_t;
...
enum {
    ...
    IRECOVERY_SEND_OPT_DFU_CRC_RETAIN          = (1 << 3),
    IRECOVERY_SEND_OPT_DFU_CRC_FINALIZE        = (1 << 4)
};
...
typedef enum {
    IRECOVERY_PROGRESS,       // Progress reports on the sending of a single continuous buffer. Use with irecovery_send_buffer().
    IRECOVERY_APPVAR_PROGRESS // Progress reports on the sending of chunked APPVARs. Use with irecovery_send_appvar().
...
/**
 * @brief Sends a TI-OS variable to the currently connected device (if any).
 * @param[in] filename Name of the variable to send.
 * @param[in] client The client to send the file to.
 * @param[in] options Send options. SEE WHAT'S SAFE TO PASS THROUGH BEFORE SETTING!
 * @param[in] more_after_this Whether or not to send DFU notify finish.
 * @return An irecovery_error_t error code.
 * 
 * Do not pass IRECOVERY_SEND_OPT_DFU_CRC_RETAIN or
 * IRECOVERY_SEND_OPT_DFU_CRC_FINALIZE — these are managed internally
 * for chunked sending.
 *
 * Do not pass IRECOVERY_SEND_OPT_DFU_NOTIFY_FINISH — send_appvar
 * applies this automatically on the final chunk. Passing it externally
 * may trigger reset mid-transfer.
 *
 * IRECOVERY_SEND_OPT_DFU_SMALL_PKT and IRECOVERY_SEND_OPT_DFU_FORCE_ZLP
